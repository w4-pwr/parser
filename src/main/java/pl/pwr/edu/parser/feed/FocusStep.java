...
public class FocusStep implements Step {
	...
	private Optional<Article> getArticle(String articleUrl) {
		try {
			...
			System.err.println("Article not fetched: " + articleUrl + " -> " + iae.getMessage());
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private HashMap<String, String> getMetadata(Document articleDocument) {
		...
		String author = getAuthor(articleDocument);
		if (isAuthorForbidden(author)) {
			throw new IllegalArgumentException("Author: " + author + " is forbidden");
		}
	...
	private boolean isAuthorForbidden(String author) {
		return forbiddenAuthors.stream().anyMatch(s -> s.equalsIgnoreCase(author));
	}
